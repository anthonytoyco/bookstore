...
/**
 * Interface defining operations for a polymorphic material store that
 * demonstrates interface segregation and
 * dependency inversion principles. Compare to {@link BookstoreAPI}, this
 * interface is extensible to other types of materials.
 * 
 * <p>
 * The Interface Segregation Principle (ISP) states that clients should not be
 * forced to depend on interfaces they
 * do not use. In practice, this means designing smaller, more focused
 * interfaces rather than large, monolithic ones.
 * For example, instead of creating a single Machine interface with methods like
 * print(), scan(), and fax(), which
 * every implementation must support, ISP encourages splitting these into
 * multiple role-specific interfaces (Printer,
 * Scanner, FaxMachine). This ensures that implementing classes only provide the
 * functionality relevant to them,
 * leading to higher cohesion, reduced coupling, and easier maintenance. ISP
 * directly combats the rigidity and
 * fragility that arise when changes in unused methods propagate across
 * unrelated parts of the system.
 * </p>
 *
 * <p>
 * The Dependency Inversion Principle (DIP) requires that high-level modules
 * depend on abstractions, not concrete
 * implementations. Instead of having business logic directly tied to low-level
 * details (like a specific database
 * driver or logging mechanism), DIP advocates for using interfaces or abstract
 * classes as boundaries. For instance,
 * a PaymentService should rely on a PaymentProcessor interface rather than a
 * hardcoded StripeProcessor. This allows
 * developers to substitute implementations without altering higher-level
 * policies, supporting testability,
 * scalability, and adaptability to new technologies. By inverting the
 * dependency direction, DIP ensures that both
 * high- and low-level modules evolve independently, fostering loosely coupled,
 * extensible architectures.
 * </p>
 *
 * <p>
 * This interface extends the concept of a bookstore to handle
 * various types of materials using polymorphism.
 * </p>
 *
    ...
    class InventoryStats {
        ...
        public String toString() {
            return String.format(
                    "InventoryStats[Total=%d, AvgPrice=$%.2f, MedianPrice=$%.2f, Types=%d, Media=%d, Print=%d]",
